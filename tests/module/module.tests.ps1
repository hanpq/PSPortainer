...
BeforeAll {
    ...
    # Must load custom types in session, otherwise ScriptParser will fail if it stumbles upon unknown classes.
    $ModuleManifest = Import-PowerShellDataFile -Path (Join-Path -Path $ModuleSourceRootPath -ChildPath ('\{0}.psd1' -f $ModuleName))
    foreach ($type in $ModuleManifest.RequiredAssemblies)
    {
        Add-Type -Path (Resolve-Path -Path (Join-Path -Path $ModuleSourceRootPath -ChildPath $type))
    }
...
Describe -Name 'Foreach script file' -Tag 'Module' -Fixture {
    ...
    It -Name 'End Of Line Type: <FileName>' -Skip:($IsMacOS -eq $true -or $IsLinux -eq $true) -TestCases $TestCases -Test {
    ...
    It -Name 'PSScriptInfo: <FileName>' -TestCases $FunctionTestCases -Test {
        New-Variable astTokens -Force -ErrorAction Stop
        New-Variable astErr -Force -ErrorAction Stop
        $null = [System.Management.Automation.Language.Parser]::ParseFile($File.FullName, [ref]$astTokens, [ref]$astErr)
        $astTokens | Where-Object { $_.Kind -eq 'Comment' -and $_.Text -like '<#PSScriptInfo*' } | Should -Not -BeNullOrEmpty
    }
